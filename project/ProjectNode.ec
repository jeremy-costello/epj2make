...
static define app = ((Application)__thisModule);
...
private:
   ...
   ProjectNode Add(Project project, const char * filePath, ProjectNode after, NodeTypes type, uint /*NodeIcons*/ icon, bool checkIfExists)
   {
      ProjectNode node = null;
   ...
   void GetTargets(ProjectConfig prjConfig, Map<CIString, NameCollisionInfo> namesInfo, char * objDir, const char * objectFileExt, ZString output)
   {
      char moduleName[MAX_FILENAME];
      if(type == file)
      {
...
static void DynStringPrintNodeFlagsVariable(ProjectNode node, Map<intptr, int> nodeFlagsMapping, const String variableName, ZString s)
{
   int customFlags;
   customFlags = nodeFlagsMapping[(intptr)node];
   if(customFlags > 1)
...
static void GenCFlagsFromProjectOptions(ProjectOptions options, bool prjWithEcFiles, bool commonOptions, bool isGreater, ZString s)
{
   if(!isGreater)
   {
      //if(gccCompiler)
...
static void GenECFlagsFromProjectOptions(ProjectOptions options, bool prjWithEcFiles, ZString s)
{
...
static void ListOptionToDynamicString(ZString output, ToolchainFlag flag, Array<String> list, bool prioritize,
